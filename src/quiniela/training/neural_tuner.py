...
def run_neural_tuning(
    ...
    objective = str(tuning_config.get("objective", "ev_mean_points"))
    trial_limit = int(max_trials or tuning_config.get("max_trials", 24))
    ...
    print(
        f"[neural-tune] trials={len(trials)} objective={objective} output={output_root}",
        flush=True,
    )
    ...
    for index, trial_config in enumerate(trials, start=1):
        ...
        print(f"[neural-tune][{trial_id}] params={_compact_params(trial_config)}", flush=True)
        existing_summary = trial_dir / "training_summary.json"
        if existing_summary.exists() and not fresh:
            print(f"[neural-tune][{trial_id}] usando resultado existente", flush=True)
        ...
        print(
            f"[neural-tune][{trial_id}] objective={row['objective_value']:.4f} "
            f"exact={row['mean_exact_accuracy']:.4f} outcome={row['mean_outcome_accuracy']:.4f} "
            f"best={best['trial_id']}:{best['objective_value']:.4f}",
            flush=True,
        )
    ...
    print(f"[neural-tune] terminado best={summary['best_trial']}", flush=True)
    return summary
...
def _trial_configs(base_config: dict[str, Any], max_trials: int) -> list[dict[str, Any]]:
    ...
    for combo in combinations[:max_trials]:
        ...
        trial_config["model_version"] = "0.2.0"
...
def _score_trial(
    ...
    return {
        ...
        "team_embedding_dim": config["training"]["team_embedding_dim"],
        "scoreline_weight": config["training"]["loss_weights"]["scoreline"],
        "quiniela_reward_weight": config["training"]["loss_weights"].get("quiniela_reward", 0.0),
    }
...
def _objective_value(folds: list[dict[str, Any]], objective: str, tuning_config: dict[str, Any]) -> float:
    ...
    min_matrix_outcome = float(guard.get("min_matrix_outcome_accuracy", 0.53))
    min_ev_outcome = float(guard.get("min_ev_outcome_accuracy", 0.53))
    matrix_penalty = max(0.0, min_matrix_outcome - matrix_outcome_accuracy)
    ev_penalty = max(0.0, min_ev_outcome - ev_outcome_accuracy)
    return (
        ev_points
        + float(guard.get("matrix_outcome_bonus", 0.35)) * matrix_outcome_accuracy
        + float(guard.get("exact_bonus", 0.15)) * exact_accuracy
        - float(guard.get("outcome_penalty", 3.0)) * (matrix_penalty + ev_penalty)
    )
...
def _compact_params(config: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"drop={training['dropout']} hidden={training['hidden_dim']} "
        f"emb={training['team_embedding_dim']} score_w={weights['scoreline']} "
        f"quiniela_w={weights.get('quiniela_reward', 0.0)}"
    )
