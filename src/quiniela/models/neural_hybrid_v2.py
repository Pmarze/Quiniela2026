...
from quiniela.features.hybrid_features import build_hybrid_prediction_features
from quiniela.features.neural_features import score_matrix_from_probs
...
class NeuralHybridV2(nn.Module):
    def __init__(
        ...
        input_dim = feature_count + team_embedding_dim * 4
        ...
        self.shared = nn.Sequential(nn.LayerNorm(hidden_dim), nn.GELU())
        ...
        self.scoreline_head = nn.Sequential(
            nn.Linear(hidden_dim + 3, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, (max_goals + 1) ** 2),
        )
    ...
    def forward(self, team_a: torch.Tensor, team_b: torch.Tensor, features: torch.Tensor) -> dict[str, torch.Tensor]:
        emb_a = self.team_embedding(team_a)
        emb_b = self.team_embedding(team_b)
        embedded = torch.cat([emb_a, emb_b, emb_a - emb_b, emb_a * emb_b, features], dim=1)
        hidden = self.shared(self.blocks(self.input(embedded)))
        outcome_logits = self.outcome_head(hidden)
        outcome_probs = torch.softmax(outcome_logits, dim=1)
        conditioned = torch.cat([hidden, outcome_probs], dim=1)
        return {
            "scoreline_logits": self.scoreline_head(conditioned),
            "outcome_logits": outcome_logits,
            "goals": self.goals_head(conditioned),
        }


def run_neural_hybrid_v2(
    ...
    if not metadata_path.exists() or not weights_path.exists():
        return [_unavailable_prediction(context, model_version, match, artifact_dir) for match in context.prediction_matches]
    ...
    outcome_temperature = artifact["outcome_temperature"]

    examples_by_match_id = {
        example.match_id: example
        for example in build_hybrid_prediction_features(context, team_vocab)
    }
    ...
    with torch.no_grad():
        for match in context.prediction_matches:
            ...
            if example is None:
                predictions.append(failed_prediction(context, MODEL_ID, model_version, match, "faltan features hibridas"))
                continue
            features = (torch.tensor([example.features], dtype=torch.float32, device=device) - feature_mean) / feature_std
            ...
            output = model(team_a, team_b, features)
            score_probs = torch.softmax(output["scoreline_logits"] / scoreline_temperature, dim=1).squeeze(0).cpu().tolist()
            outcome_probs = torch.softmax(output["outcome_logits"] / outcome_temperature, dim=1).squeeze(0).cpu().tolist()
            ...
            adjusted_matrix = adjust_score_matrix_to_1x2(
                score_matrix,
                {"1": outcome_probs[0], "X": outcome_probs[1], "2": outcome_probs[2]},
            )
            selected = select_best_score(adjusted_matrix, scoring_config)
            predictions.append(
                successful_prediction_from_matrix(
                    context=context,
                    model_id=MODEL_ID,
                    ...
                    warnings=["matriz calibrada por head 1X2"],
                )
            )
    return predictions
...
def _artifact_dir(db_path: Path, model_config: dict[str, Any]) -> Path:
    raw = Path(str(model_config.get("artifact_dir", "data/models/neural_hybrid_v2/latest")))
    if raw.is_absolute():
        return raw
    return db_path.resolve().parents[1] / raw


def _load_artifact(artifact_dir: Path, metadata_path: Path, weights_path: Path, device: torch.device) -> dict[str, Any]:
    ...
    model = NeuralHybridV2(
        ...
        num_blocks=int(model_params["num_blocks"]),
    ...
    artifact = {
        ...
        "outcome_temperature": max(0.05, float(calibration.get("outcome_temperature", 1.0))),
        "model": model,
    }
...
def _unavailable_prediction(context: ModelContext, model_version: str, match: Any, artifact_dir: Path) -> ModelPrediction:
    mask_reason = mask_reason_for_match(match)
    if mask_reason:
    ...
    return failed_prediction(
        ...
        model_version=model_version,
        match=match,
        warning=f"artifacto neural_hybrid_v2 no encontrado: {artifact_dir}",
    )
